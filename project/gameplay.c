...
void initGame(int nbPlayers) {
	...
	for (i = 1; i <= nbPlayers; ++i) {
		switch (i) {
		case 1 :
			initPoint2D(&barCenter, (screenWidthCenter - (BASIC / 2)), BAR_HEIGHT);
			initColor3f(&barColor, 255, 0, 0);
			initPoint2D(&ballCenter, screenWidthCenter, (BAR_HEIGHT * 2));
			initVector2D(&ballSpeed, NORMAL, NORMAL);
			initColor3f(&ballColor, 255, 0, 0);
			break;
		case 2 :
			initPoint2D(&barCenter, (screenWidthCenter - (BASIC / 2)), (SCREEN_HEIGHT - BAR_HEIGHT));
			initColor3f(&barColor, 0, 0, 255);
			initPoint2D(&ballCenter, screenWidthCenter, (SCREEN_HEIGHT - (BAR_HEIGHT * 2)));
...
void ballOutOfBounds(Ball *ball, enum direction dir) {
	...
	if (dir == BOTTOM) {
		--(players[1].life);
		ball->lastPlayerId = 1;
		ball->origin.y = SCREEN_HEIGHT - 60;
	}
}
